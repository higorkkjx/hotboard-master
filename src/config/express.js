...
app.post('/uploadgit', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const response = await axios.put(
      `https://api.github.com/repos/${github.GITHUB_USERNAME}/${github.GITHUB_REPO}/contents/${req.file.originalname}`,
      {
        message: 'Upload de vídeo via API',
        content: base64File
      },
      {
        headers: {
    ...
    const videoUrl = response.data.content.download_url;
    res.json({ url: videoUrl });

    // Limpar o arquivo temporário
    fs.unlinkSync(req.file.path);
  } catch (error) {
    console.error('Erro ao fazer upload para o GitHub:', error);
    res.status(500).send('Erro ao fazer upload do vídeo');
  }
});

// Handles any requests that match the defined routes
app.get('/gvideo', (req, res) => {
...
<body>
    <h1>Upload de Vídeo para GitHub</h1>
    <form action="/uploadgit" method="post" enctype="multipart/form-data">
