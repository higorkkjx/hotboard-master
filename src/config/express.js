...
const getInstanceInfo = async (chave) => {
    const instanceInfoUrl = `https://195.200.7.87:3000/instance/info?key=${chave}`;
...
app.get('/whats/:chave', async (req, res, next) => {
    const chave = req.params.chave;
    try {
        const chatsMsgs = await fetch(`https://195.200.7.87:3000/chats/${chave}`);
     ...
     //  }
      ...
      //  sortChatsByLastMessageTime(chatsdata.chatsdata);
        ...
        res.render("whats3", {
            ...
            chave,
            profileImageUrl,
            urlapi: "https://195.200.7.87:3000"
        });
    } catch (error) {
...
app.get('/chats/:chave', async (req, res, next) => {
  try {
    ...
    const chatResponse = await fetch(`https://195.200.7.87:3000/instance/gchats?key=${chave}`);
    if (!chatResponse.ok) {
    ...
    try {
      ...
      const chat = chatsData.find(chat => chat.id === chatNum);
       ...
       const chatsData = snapshot.map(doc => ({ id: doc._id, data: doc }));
        ...
        const { nomezap, numeroid } = await getInstanceInfo(chave);
        const funisResponse = await fetch(`https://195.200.7.87:3000/instance/displayallfunis?key=${chave}`);
        if (!funisResponse.ok) {
    ...
    <body class="bg-gray-100">
        ...
        <script>
            document.getElementById('deleteAccessForm').addEventListener('submit', async function(event) {
                ...
                try {
                    const response = await fetch('https://195.200.7.87:3000/instance/delete?key=' + key);
                    if (response.ok) {
