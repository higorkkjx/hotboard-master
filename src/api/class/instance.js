...
const autoresp_config = await axios.get(`http://evolucaohot.online/autoresposta/dados/${this.key}/${m.messages[0].key.remoteJid.replace("@s.whatsapp.net", "")}`)       
       ...
       try {
              ...
              const checkAndAddChat = async (sender, pushname, body, fromme, gppessoanome) => {
                try {
                    ...
                    const profileResponse = await fetch(`http://evolucaohot.online/misc/downProfile?key=${this.key}`, {
                        method: 'POST',
                        ...
                        if (currentFunilStep && currentFunilStep.tipoMensagem === "choice") {
                            ...
                            if (!isNaN(userChoice) && userChoice > 0 && userChoice <= currentFunilStep.conteudo.opcoes.length) {
                                ...
                                async function salvarFunil(key, funilSelecionado) {
                                    const url = `http://evolucaohot.online/api/salvar-funil-user/${key}/${m.messages[0].key.remoteJid.replace("@s.whatsapp.net", "")}`
                                    const data = {
...
async enviarAudio(key, linkDoAudio, id, tipoUsuario, delay) {
    try {
        ...
        const result = await axios.post(`http://evolucaohot.online/message/audiofile?key=` + key, formData);
