...
nomearqv = filename+ ".mp4"
      ...
      console.error(`Erro ao hospedar o arquivo no GitHub:`, error.message);
    } finally {
  ...
  switch (messageType) {
    case 'imageMessage':
      mediaUrl = await uploadMediaToGithub(msg.message.imageMessage, 'image', github);
      var dadomsg = {
        sessao: this.key,
 ...
 await this.checkAndAddChat(dadomsg.chat, dadomsg.pushname, mediaUrl, dadomsg.fromMe, dadomsg.pushname)
      break;
      case 'stickerMessage':
        mediaUrl = await uploadMediaToGithub(msg.message.stickerMessage, 'image', github);
        var dadomsg = {
          sessao: this.key,
          chat: msg.key.remoteJid,
          fromMe: msg.key.fromMe,
   ...
   await this.checkAndAddChat(dadomsg.chat, dadomsg.pushname, mediaUrl, dadomsg.fromMe, dadomsg.pushname)
        break;
    case 'audioMessage':
      mediaUrl = await uploadMediaToGithub(msg.message.audioMessage, 'audio', github);
      var dadomsg = {
        sessao: this.key,
        chat: msg.key.remoteJid,
        fromMe: msg.key.fromMe,
 ...
 await this.checkAndAddChat(dadomsg.chat, dadomsg.pushname, mediaUrl, dadomsg.fromMe, dadomsg.pushname)
      break;
    case 'videoMessage':
      mediaUrl = await uploadMediaToGithub(msg.message.videoMessage, 'video', github);
      var dadomsg = {
        sessao: this.key,
        chat: msg.key.remoteJid,
        fromMe: msg.key.fromMe,
...
async hospedarArquivo(msg, tipo) {
    ...
    try {
      ...
      const messageContent = msg.message[`${tipo}Message`];
      console.log(messageContent)
   
  
      // Download the message content and get the media path
      mediaPath = await downloadMessage(messageContent, tipo);
      console.log(`Media path for ${tipo}:`, mediaPath);
  
      // Verify the mediaPath and check if the file exists
      if (!mediaPath || !fs2.existsSync(mediaPath)) {
        throw new Error(`Caminho do arquivo de ${tipo} inválido ou o arquivo não existe.`);
      }
  
      // Read the file and convert it to base64
      const base64File = fs2.readFileSync(mediaPath).toString('base64');
  
      // Upload the file to GitHub
      const response = await axios.put(
        `https://api.github.com/repos/${github.GITHUB_USERNAME}/${github.GITHUB_REPO}/contents/${path.basename(mediaPath)}`,
        {
          message: `Upload de ${tipo} via API`,
          content: base64File,
        },
        {
          headers: {
      ...
      console.log(`${tipo} hospedado com sucesso no GitHub:`, mediaUrl);
      return mediaUrl;
    } catch (error) {
      console.error(`Erro ao hospedar o arquivo no GitHub:`, error.message);
      return null;
    ...
    try {
        ...
        for await (const [id, value] of this.datab.iterator()) {
            ...
            try {
                ...
                console.warn(`Erro ao analisar o valor para o chat ${id}:`, parseError);
        ...
        const sortedChats = chats.sort((a, b) => {
            ...
            const bLastMessage = b.data.mensagens && b.data.mensagens.length > 0 ? b.data.mensagens[b.data.mensagens.length - 1] : null;

            if (!aLastMessage) return 1;  // a should come after b
            if (!bLastMessage) return -1; // b should come after a

            const aDate = DateTime.fromFormat(aLastMessage.data, "yyyy-MM-dd HH:mm", { zone: "America/Sao_Paulo" });
            const bDate = DateTime.fromFormat(bLastMessage.data, "yyyy-MM-dd HH:mm", { zone: "America/Sao_Paulo" });
